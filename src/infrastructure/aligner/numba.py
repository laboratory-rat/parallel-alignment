...
@njit
def get_pair_score(a: str, b: str) -> float:
    match_score = 1
    familiar_replace_panalty = -.5
    non_familiar_replace_penalty = -1.5
    familiar_pairs = [('A', 'G'), ('C', 'T'), ('C', 'U')]
    if a == b:
        return match_score

    if (a, b) in familiar_pairs or (b, a) in familiar_pairs:
        return familiar_replace_panalty

    return non_familiar_replace_penalty


@njit
def needleman_wunsch(gap_penalty, seq1, seq2) -> Tuple[str, str, float]:
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            match = score[i - 1][j - 1] + get_pair_score(seq1[i - 1], seq2[j - 1])
    ...
    while i > 0 and j > 0:
        ...
        if score_current == score_diagonal + get_pair_score(seq1[i - 1], seq2[j - 1]):
            align1 += seq1[i - 1]
            align2 += seq2[j - 1]
            i -= 1
            j -= 1
